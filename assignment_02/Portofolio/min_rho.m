function [ w, x_lambda, r_final, sigma, optim, lambda, ef] = m_rho(rho, allow_short_selling)
    ...
    if allow_short_selling == 1
      ...
      x_lambda = [];
      [w, fv, ef, optim, lambda] = quadprog(Q, [], A,b, Aeq, beq,[],[],[],options);
    else
      x_lambda = [];
      [w, fv, ef, optim, lambda] = quadprog(Q, [], A, b, Aeq, beq, zeros(n,1), [],[], options);
    end
    sigma = 1/2*w.'*Q*w;
    r_final = r*w;
    end
